...
const userDataDir = mkdtempSync(join(tmpdir(), 'bikefit-chrome-'))
const chrome = spawn(
  chromeBin,
  [
    '--headless=new',
...
try {
  ...
  const text = await withCdp(page.webSocketDebuggerUrl, async (send) => {
    ...
    while (Date.now() < deadline) {
      ...
      if (value.includes('FILE_MOUNTED_OK') || value.includes('FILE_MOUNTED_FAIL')) return value
    }
    const fallback = await send('Runtime.evaluate', {
      expression: 'document.getElementById("out")?.textContent ?? document.body?.innerText ?? ""',
      returnByValue: true,
    })
    return String(fallback.result?.result?.value ?? 'FILE_MOUNTED_FAIL — timeout')
  })

  console.log(text)
  if (!text.includes('FILE_MOUNTED_OK') || text.includes('FILE_MOUNTED_FAIL')) {
    throw new Error(text.split('\n').find((line) => line.includes('FILE_MOUNTED_FAIL')) ?? 'FILE_MOUNTED_FAIL')
  }
  console.log('FILE_MOUNTED_OK')
} finally {
  kill(chrome)
  await server.close()
}
