...
const PORT = 47323
const CDP_PORT = 9224
const HARNESS_URL = `http://127.0.0.1:${PORT}/capture-harness.html`
...
const userDataDir = mkdtempSync(join(tmpdir(), 'bikefit-capture-chrome-'))
const chrome = spawn(
  chromeBin,
  [
    '--headless=new',
...
try {
  ...
  const text = await withCdp(page.webSocketDebuggerUrl, async (send) => {
    ...
    while (Date.now() < deadline) {
      ...
      if (value.includes('CAPTURE_MOUNTED_OK') || value.includes('CAPTURE_MOUNTED_FAIL')) return value
    }
    const fallback = await send('Runtime.evaluate', {
      expression: 'document.getElementById("out")?.textContent ?? document.body?.innerText ?? ""',
      returnByValue: true,
    })
    return String(fallback.result?.result?.value ?? 'CAPTURE_MOUNTED_FAIL — timeout')
  })

  console.log(text)
  if (!text.includes('CAPTURE_MOUNTED_OK') || text.includes('CAPTURE_MOUNTED_FAIL')) {
    throw new Error(text.split('\n').find((line) => line.includes('CAPTURE_MOUNTED_FAIL')) ?? 'CAPTURE_MOUNTED_FAIL')
  }
  console.log('CAPTURE_MOUNTED_OK')
} finally {
  kill(chrome)
  await server.close()
}
