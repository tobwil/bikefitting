...
export function formatDecisionNote(report: PoseCompareReport): string {
  ...
  return [
    ...
    `CLIP      ${report.clip.kind} · ${report.clip.name} · ${report.clip.frames} frames · ${report.clip.width}×${report.clip.height} · detector=${report.detector}`,
  ].join('\n')
}
