...
Status: **this branch**. Automatic point proposals + confirm + easy correct. Fully unsupervised calib is out of scope. Findings 1–3 (gold prototype honesty, off-thread detect, image generation) are in.

## BUILD_OK

`npm run build` plus:

- `npm run calib:harness` — propose → confirm → apply, gold-rectangle reject, pixel occlusion, shifted-still generation, O(N) facing
